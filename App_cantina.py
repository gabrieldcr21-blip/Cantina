...
def obtener_tasa_activa(config: dict) -> float:
    ...
    try:
        if tipo == "BCV USD":
            return float(config.get("tasa_bcv_usd", 0) or 0)
        if tipo == "BCV EUR":
            return float(config.get("tasa_bcv_eur", 0) or 0)
        if tipo == "Personalizada":
            return float(config.get("tasa_personalizada", 0) or 0)
...
def generar_enlace_whatsapp(telefono: str, mensaje: str) -> str:
    ...
    if not tel:
        return ""
    return "https://wa.me/" + tel + "?text=" + urllib.parse.quote(mensaje)
...
def init_session_state():
    ...
    if "cliente_sel_id" not in st.session_state:
        st.session_state.cliente_sel_id = 0


def carrito_add(pid: int, prod_row):
...
def render_metric(label: str, value: str, variante: str = ""):
    st.markdown(
        f"""
        <div class="metric-card {variante}">
...
# 7. MÓDULO POS
# ============================================================
def _finalizar_venta(estado: str, tasa: float, total_usd: float, total_bs: float):
    if carrito_vacio():
    ...
    if cid and cid != 0:
        ...
        if not row.empty:
            ...
            cid, cliente_nombre, alumno, representante = 0, "Anónimo", "", ""
    else:
        cid, cliente_nombre, alumno, representante = 0, "Anónimo", "", ""

    detalles = carrito_detalle_txt()
    ...
    if estado == "Pagado":
        st.success(f"✅ Venta registrada y PAGADA · ${total_usd:.2f} / Bs. {total_bs:,.2f}")
    else:
        st.warning(f"🔴 Venta FIADA registrada · ${total_usd:.2f} / Bs. {total_bs:,.2f}")


def modulo_pos(config: dict):
    tasa = obtener_tasa_activa(config)

    prod_df = get_productos()
    if prod_df.empty:
        st.warning("No hay productos. Agrega productos en la pestaña '➕ Productos'.")
        return

    categorias = ["Todas"] + sorted(prod_df["categoria"].dropna().unique().tolist())
